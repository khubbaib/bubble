...
class BubbleOverlayService : Service() {
    ...
    fun updateIconTop(iconUrl: String?) {
        val iconTop = mBubbleView?.findViewById<ImageView>(R.id.bubble_image_top)
        if (iconTop != null && iconUrl != null) {
            Glide.with(this).load(Uri.parse(iconUrl)).into(iconTop)
        }
        else
            iconTop?.setImageResource(0)
    }

    fun updateIconBottom(iconUrl: String?) {
        val iconBottom = mBubbleView?.findViewById<ImageView>(R.id.bubble_image_bottom)
        if (iconBottom != null && iconUrl != null)
            Glide.with(this).load(Uri.parse(iconUrl)).into(iconBottom)
        else
